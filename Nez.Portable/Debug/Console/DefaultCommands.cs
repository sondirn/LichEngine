...
namespace Nez.Console
{
	public partial class DebugConsole
	{
		...
		[Command("inspect", "Inspects the entity with the passed Id, pass in -1 to close the inspector.")]
		static void InspectEntityId(int Id = -1)
		{
			if(Instance._runtimeInspector != null)
			{
				Instance._runtimeInspector.Dispose();
			...
			if(Id != -1)
			{
				var entity = Core.Scene.FindEntity(Id);
				if(entity == null)
				{
					Instance.Log("could not find eneity with id of " + Id);
					return;
				}
				Instance._runtimeInspector = new RuntimeInspector(entity);
				Instance.IsOpen = false;
